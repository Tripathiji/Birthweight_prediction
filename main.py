# -*- coding: utf-8 -*-
"""
Created on Sun Mar  3 17:01:04 2019
...
# Everything will be filled with the median
...
birthweight['smokeo'] = 0

for val in enumerate(birthweight.loc[ : , 'cigs']):
    
    if val[1] == 0:
        birthweight.loc[val[0], 'smokeo'] = 0
    else: birthweight.loc[val[0], 'smokeo'] = 1

birthweight['drinko'] = 0

for val in enumerate(birthweight.loc[ : , 'drink']):
    
    if val[1] == 0:
        birthweight.loc[val[0], 'drinko'] = 0
    else: birthweight.loc[val[0], 'drinko'] = 1

birthweight['bad_sum'] = birthweight['smokeo'] + birthweight['drinko'] 
# Smoke + Drink

birthweight['smokeo'] = 0


for val in enumerate(birthweight.loc[ : , 'bad']):
...
file = 'birthweight_explored1.xlsx'
...
birthweight_3['smokeo'] = 0

for val in enumerate(birthweight_3.loc[ : , 'cigs']):
    
    if val[1] == 0:
        birthweight_3.loc[val[0], 'smokeo'] = 0
    else: birthweight_3.loc[val[0], 'smokeo'] = 1

birthweight_3['drinko'] = 0

for val in enumerate(birthweight_3.loc[ : , 'drink']):
    
    if val[1] == 0:
        birthweight_3.loc[val[0], 'drinko'] = 0
    else: birthweight_3.loc[val[0], 'drinko'] = 1

birthweight_3['bad_sum'] = birthweight_3['smokeo'] + birthweight_3['drinko'] 
#############################################################################

sns.distplot(birthweight_3['mon'],
             bins = 17,
             kde = False,
             rug = True,
             color = 'r')
...
page_high= 95
birthweight_3['out_page'] = 0

for val in enumerate(birthweight_3.loc[ : , 'page']):
    
    if val[1] >= page_high:
        birthweight_3.loc[val[0], 'out_page'] = 1

pduc_low=18
pduc_high=28
birthweight_3['out_pduc'] = 0

for val in enumerate(birthweight_3.loc[ : , 'pduc']):
    
    if val[1] >= pduc_high:
        birthweight_3.loc[val[0], 'out_pduc'] = 1
...
mon_high=6
birthweight_3['out_mon']=0

for val in enumerate(birthweight_3.loc[ : , 'mon']):
    
    if val[1] >= mon_high:
        birthweight_3.loc[val[0], 'out_mon'] = 1
        
birthweight_3['bad'] = birthweight_3.drink + birthweight_3.cigs
birthweight_3['bad1'] = birthweight_3.bad.map( lambda x: 1 if x > 0 else 0)

birthweight_3.to_excel('birthweight_explored1.xlsx')
...
# The best results occur when k = 6.
...
print(f"""
Optimal model KNN score:    {y_score.round(3)}
...
birthweight_tree = pd.read_excel(file)

birthweight_data1 = birthweight_3.loc[:,['monpre',
                                       'mon',
                                       'bad1',
                                       'mage',
                                       'pduc',
...
# Creating a tree with only three levels.
...
export_graphviz(decision_tree = tree_2_fit,
                ...
                feature_names = birthweight_data1.columns)
...
########################
# Let's adjust the some model arguments of tree before moving forward.
########################
...
graph.write_png("Ames_Decision_Tree.png")


"""
Prof. Chase:
    While this tree could be much deeper, at its current depth it can be
    overwhelming for some audiences, especially when trying to develop
    insights. To assist with this, a common approach summarize using feature
    importance. This technique rates each feature in terms of how much it was
    used in developing the tree.
"""

print(tree_leaf_50.feature_importances_)
...
def plot_feature_importances(model, train = X_train, export = False):
    ...
    if export == True:
        plt.savefig('birthweight_Leaf_50_Feature_Importance.png')
########################
